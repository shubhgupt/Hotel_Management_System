...
public class AddDriver extends JFrame implements ActionListener{
    ...
    AddDriver(){
        ...
        location.setBounds(50, 340, 110, 25 );
        location.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(location);
        t5 = new JTextField();
        t5.setBounds(190, 340, 170, 25);
        ...
        Image I = i1.getImage().getScaledInstance(480, 390, Image.SCALE_DEFAULT);
        i1 = new ImageIcon(I);
        JLabel image = new JLabel(i1);
        image.setBounds(390, 40, 480, 390);
        add(image);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                ...
                if(age < 18 || age > 70 || name.equals("") || gender.equals("") || company.equals("") || model.equals("")
                        || location.equals("")){
                    JOptionPane.showMessageDialog(rootPane, "Invalid Input!", "Error", JOptionPane.ERROR_MESSAGE);
                }else {

                    Conn c = new Conn();
                    String query = "Insert into drivers(name, age, gender, CarCompany, model, availability, location) values( ?, ?, ?, ?, ?, ?, ?)";
                    PreparedStatement pt = c.con.prepareStatement(query);
                    pt.setString(1, name);
                    pt.setInt(2, age);
                    pt.setString(3, gender);
                    pt.setString(4, company);
                    pt.setString(5, model);
                    pt.setString(6, available);
                    pt.setString(7, location);

                    int surity = JOptionPane.showConfirmDialog(rootPane, "Please Confirm!");
                    if (surity == JOptionPane.YES_OPTION) {
                        int result = pt.executeUpdate();
                        if (result == 1) {
                            JOptionPane.showMessageDialog(rootPane, "Driver Added SuccessFully!", "Success", JOptionPane.PLAIN_MESSAGE);
                            clearAllFields();
                        } else {
